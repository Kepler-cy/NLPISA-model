...
def Eu_dis(x):
    ...
    ab = x * x.T  #
    dist_mat = aa + aa.T - 2 * ab  # aa + aa.T = (12311, 12311);
...
def feature_concat(*F_list, normal_col=False):
    ...
    for f in F_list:  # (fts, gvcnn_ft)
        if f is not None and f != []:
            ...
            if len(f.shape) > 2:
                f = f.reshape(-1, f.shape[-1])  # trans to 2 dim
            # normal each column
            if normal_col:
...
def _generate_G_from_H(H, variable_weight=False):
    ...
    :return: G
    """
    H = np.array(H)  # hypeedge
    n_edge = H.shape[1]
    # the weight of the hyperedge
    W = np.ones(n_edge)
    # the degree of the node
    DV = np.sum(H * W, axis=1)  # degree
...
def construct_H_with_KNN_from_distance(dis_mat, k_neig, is_probH=True, m_prob=1):
    ...
    for center_idx in range(n_obj):
        dis_mat[center_idx, center_idx] = 0  #self= 0;
        ...
        #consturt hedge
        for node_idx in nearest_idx[:k_neig]:  # ten nodes
            if is_probH:
